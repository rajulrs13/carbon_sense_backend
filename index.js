...
function convertKoMo(number) {
  numberKo = (number / 1024).toFixed(0);
  if (numberKo >= 1024) return (numberKo / 1024).toFixed(2) + " Mo";
  else return (number / 1024).toFixed(0) + " Ko";
}
...
//       nbRequest++;
//       if (typeof data.response.headers["content-length"] !== "undefined")
//         contentLength += parseInt(data.response.headers["content-length"]);
//     }
//   });

//   client.on("Network.loadingFinished", async (data) => {
//     if (typeof data.encodedDataLength !== "undefined")
//       encodedDataLength += parseInt(data.encodedDataLength);
//   });

//   await page.goto("https://dataops-frontend-dev.r1.pcf.dell.com", {
...
async function run(url_to_scan) {
  ...
  //   await page.screenshot({ path: 'forbes.png' });
